/* Progressive enhancement: copy the contract address.
   The site is fully usable without this script. */
(function () {
  var button = document.getElementById("copy-address");
  var address = document.getElementById("contract-address");
  if (!button || !address || !navigator.clipboard) return;

  button.hidden = false;
  var timer = null;

  button.addEventListener("click", function () {
    navigator.clipboard.writeText(address.textContent.trim()).then(function () {
      button.textContent = "Copied";
      clearTimeout(timer);
      timer = setTimeout(function () {
        button.textContent = "Copy";
      }, 2000);
    });
  });
})();

/* In-memory language switch. English remains the default on every page load. */
(function () {
  var language = "en";
  var options = document.querySelectorAll("[data-language]");
  var copy = document.querySelectorAll("[data-en][data-lt]");
  if (!options.length) return;

  function setLanguage(nextLanguage) {
    language = nextLanguage === "lt" ? "lt" : "en";
    document.documentElement.lang = language;

    copy.forEach(function (element) {
      element.textContent = element.dataset[language];
    });

    options.forEach(function (option) {
      var isActive = option.dataset.language === language;
      option.classList.toggle("is-active", isActive);
      option.setAttribute("aria-pressed", String(isActive));
    });
  }

  options.forEach(function (option) {
    option.addEventListener("click", function () {
      setLanguage(option.dataset.language);
    });
  });

  setLanguage(language);
})();

/* Donation note scaffold: fetch a note from the Pages Function and show it.
   The click itself is tracked via the data-sentry-action attribute. */
(function () {
  var button = document.getElementById("get-card");
  var dialog = document.getElementById("card-dialog");
  var body = document.getElementById("card-dialog-body");
  var close = document.getElementById("card-dialog-close");
  if (!button || !dialog || !body || !close || typeof dialog.showModal !== "function") return;

  function line(text, className) {
    var p = document.createElement("p");
    p.className = className || "dialog-message";
    p.textContent = text;
    return p;
  }

  function render(nodes) {
    while (body.firstChild) body.removeChild(body.firstChild);
    nodes.forEach(function (node) { body.appendChild(node); });
    if (!dialog.open) dialog.showModal();
  }

  button.addEventListener("click", function () {
    button.disabled = true;
    fetch("/api/card").then(function (response) {
      return response.json();
    }).then(function (data) {
      if (!data || !data.ok) throw new Error("unavailable");
      render([
        line(data.message),
        line(data.serial, "dialog-message address")
      ]);
    }).catch(function () {
      render([line("Donation note is temporarily unavailable.")]);
    }).finally(function () {
      button.disabled = false;
    });
  });

  close.addEventListener("click", function () { dialog.close(); });
  dialog.addEventListener("click", function (event) {
    if (event.target === dialog) dialog.close();
  });
})();
